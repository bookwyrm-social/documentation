...
O acesso do utilizador a diferentes funcionalidades é controlado através do [sistema de autenticação integrado](https://docs.djangoproject.com/en/3.2/topics/auth/default/) do Django. When an instance is created, the `initdb` script creates a set of permissions, which are assinged to groups. By default, all new users are assigned the `editor` group, which allows them to edit book metadata.
