...
class CustomHTMLParser(HTMLParser):
    ...
    def _is_header(self, tag):
        ...
        return len(tagname) == 2 and tagname[0].lower() == 'h' and '1' <= tagname[1] <= '6'
    ...
    def handle_data(self, data):
        ...
        if self._is_header(self.get_starttag_text()) and data.strip() != '':
            ...
            self.data.append(f'{new_tag}{data}<a href=#{id_slug} class="icon icon-chain pl-3"><span class="is-sr-only">Hyperlink to this header</span></a>')
        else:
...
def register():
    ...
    # unfortunately pelican doesn't have a signal that lets us asct on parsed-but-not-transformed articles
